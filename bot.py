...
async def sender(msg, download_location, fileName):
  await msg.reply_document(download_location,
                           caption=f"{fileName}",
                           quote=True)
...
async def download(msg, download_location, url, formatt):
  try:
    command_to_exec = [
      "youtube-dl", "-f", formatt, "-o", download_location, url
    ]
...
async def getting_filename(url, msg):
  command_to_exec = [
    "youtube-dl", "--no-warnings", "--youtube-skip-dash-manifest", "-j", url
  ]
  # logger.info(command_to_exec)
  process = await asyncio.create_subprocess_exec(
    *command_to_exec,
...
async def downloader(bot: Client, msg: Message):
  ...
  print(download_location, url, formatt)
  await download(msg=msg,
                 download_location=download_location,
                 url=url,
                 formatt=formatt)
